  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        {(['', 'ativo', 'inativo', 'vendido'] as const).map(s => (
          <button
            key={s}
          ...
          >
            {s === '' ? 'Todos' : s.charAt(0).toUpperCase() + s.slice(1)} ({s === '' ? demos.length : demos.filter(d => d.status === s).length})
          </button>
        ))}
        <button
          onClick={openNew}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 grid grid-cols-2 gap-3">
              ...
              {field('data_entrada', 'Data Entrada', 'date')}
              {field('data_saida', 'Data Saída', 'date')}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-card border border-border rounded-xl shadow-xl p-6 w-full max-w-sm text-center space-y-4">
            <p className="text-sm font-medium">Tens a certeza que queres eliminar este demonstrador?</p>
        ...
        <div className="overflow-auto rounded-lg border border-border">
              ...
              {filtered.map(d => (
                <tr key={d.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-2 py-1.5">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${STATUS_BADGE[d.status]}`}>
                      {d.status}
                    </span>
                  </td>
                  <td className="px-2 py-1.5 text-muted-foreground">{d.responsavel || '—'}</td>
                  <td className="px-2 py-1.5 whitespace-nowrap text-muted-foreground">
                    {d.data_entrada ? new Date(d.data_entrada).toLocaleDateString('pt-PT') : '—'}
                  </td>
                  <td className="px-2 py-1.5 whitespace-nowrap text-muted-foreground">
                    {d.data_saida ? new Date(d.data_saida).toLocaleDateString('pt-PT') : '—'}
                  </td>
                  <td className="px-2 py-1.5 max-w-[160px] truncate text-muted-foreground">{d.observacoes || '—'}</td>
                  <td className="px-2 py-1.5">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(d)} className="p-1 text-muted-foreground hover:text-foreground transition-colors">
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => setDeleteId(d.id)} className="p-1 text-muted-foreground hover:text-destructive transition-colors">
          ...
          {filtered.length === 0 && (
            <div className="py-10 text-center text-xs text-muted-foreground">
              Nenhum demonstrador {statusFilter && `com estado "${statusFilter}"`} encontrado.
            </div>
          )}
        </div>
      )}
