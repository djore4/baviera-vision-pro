...
export default function RetailsPage() {
  ...
  // Base: only Carteira, Matricula, Retail statuses with VN/VD
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [baseRecords, selectedResp, selectedGar]);

  const retails = useMemo(() => filtered.filter(r => r.status === 'Retail'), [filtered]);

  // Status por Responsável — counts by resp for the 3 statuses
  const statusByResp = useMemo(() => {
    const map: Record<string, { resp: string; Carteira: number; Matricula: number; Retail: number }> = {};
    filtered.forEach(r => {
      if (!map[r.resp]) map[r.resp] = { resp: r.resp, Carteira: 0, Matricula: 0, Retail: 0 };
    ...
    return Object.values(map).sort((a, b) => (b.Retail + b.Carteira + b.Matricula) - (a.Retail + a.Carteira + a.Matricula));
  }, [filtered]);

  const totalFiltered = useMemo(() => filtered.length, [filtered]);
  ...
  const handleRespClick = useCallback((respName: string) => {
    setSelectedResp(prev => prev === respName ? null : respName);
    setPage(0);
  }, []);

  const handleGarClick = useCallback((garName: string) => {
    setSelectedGar(prev => prev === garName ? null : garName);
    setPage(0);
  }, []);
      ...
      <div className="flex-1 min-w-0 space-y-3">
        ...
        {activeFilters.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            {selectedResp && (
              <Badge variant="secondary" className="text-[10px] cursor-pointer" onClick={() => setSelectedResp(null)}>
                {selectedResp} ✕
              </Badge>
            )}
            {selectedGar && (
              <Badge variant="secondary" className="text-[10px] cursor-pointer" onClick={() => setSelectedGar(null)}>
                {selectedGar} ✕
              </Badge>
            )}
          </div>
        ...
        <div className="grid grid-cols-12 gap-3">
          ...
          <div className="col-span-5 bg-card border border-border rounded-lg p-3">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={statusByResp} barSize={12} onClick={(e) => { if (e?.activeLabel) handleRespClick(e.activeLabel as string); }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                <XAxis dataKey="resp" tick={{ fontSize: 10, cursor: 'pointer' }} />
                <YAxis tick={{ fontSize: 10 }} />
                <Tooltip contentStyle={{ fontSize: 11, background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))' }} />
                <Legend wrapperStyle={{ fontSize: 10 }} />
                <Bar dataKey="Retail" stackId="a" fill="#1C69D4" label={{ position: 'top', fontSize: 8 }} />
          ...
          <div className="col-span-4 bg-card border border-border rounded-lg p-3">
            ...
            <div className="flex items-center">
              <ResponsiveContainer width="60%" height={200}>
                <PieChart>
                  <Pie data={finData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={75} label={({ pct }) => `${pct}%`} labelLine={false}>
                    {finData.map((entry, i) => (
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number, name: string) => [`${value} (${finData.find(f => f.name === name)?.pct || 0}%)`, name]}
                  />
                </PieChart>
              </ResponsiveContainer>
              <div className="w-[40%] space-y-1.5">
                {finData.map((entry, i) => (
                  <div key={entry.name} className="flex items-center gap-1.5">
                    <span className="w-2.5 h-2.5 rounded-sm flex-shrink-0" style={{ backgroundColor: FIN_COLORS[entry.name] || COLORS[i % COLORS.length] }} />
                    <span className="text-[10px] flex-1">{entry.name}</span>
                    <span className="text-[10px] font-semibold">{entry.value}</span>
                    <span className="text-[10px] text-muted-foreground">({entry.pct}%)</span>
                  </div>
                ))}
                <div className="border-t border-border pt-1 mt-1">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] font-semibold flex-1">Total</span>
        ...
        <div className="grid grid-cols-12 gap-3">
          ...
          <div className="col-span-2 space-y-3">
            <DonutCard title="QoR" count={qorCount} total={filtered.length} color="#F59E0B" />
            <DonutCard title="BEV" count={bevCount} total={filtered.length} color="#16A34A" />
          </div>
