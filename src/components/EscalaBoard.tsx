 ...
 * persistência, equipa por defeito, tipologias e existência da coluna Genius)
 * vêm da `config`, para as duas escalas ficarem sempre em sincronia.
 * ──────────────────────────────────────────────────────────────────────────── */
...
export type Typology = 'Genius' | 'STAND' | 'APOIO' | 'LIVRE' | 'FOLGAS' | 'FÉRIAS' | 'FORMAÇÃO';
...
export interface Member {
  id: string;
  initials: string;
  kind: MemberKind; // PG = Genius (só coluna Genius); VEND = vendedor (restantes colunas)
}

export interface EscalaConfig {
  storagePath: string;      // ficheiro no bucket (VN e VU são distintos)
  defaultTeam: Member[];
...
function defaultState(config: EscalaConfig): EscalaState {
  return {
    version: 2,
    year: 2026,
...
function migrate(parsed: Record<string, unknown>, config: EscalaConfig): EscalaState {
  ...
  const team: Member[] = Array.isArray(parsed.team) && parsed.team.length
    ? (parsed.team as Array<Record<string, unknown>>).map(m => ({
        ...
        kind: config.hasGenius && (m.kind === 'PG' || String(m.initials) === 'JD') ? 'PG' : 'VEND',
      }))
    : base.team;
  ...
  for (const [day, dayVal] of Object.entries(rawAssign)) {
    ...
    for (const [k, v] of Object.entries(dayVal)) {
      if (Array.isArray(v)) {
        ...
        if (config.typologies.includes(typ)) dayMap[typ] = v as string[];
      } else if (typeof v === 'string' && v) {
        const typ = (v === 'FOLGA' ? 'FOLGAS' : v === 'PG' ? 'Genius' : v) as Typology;
        if (config.typologies.includes(typ)) {
          (dayMap[typ] ??= []).push(k);
        }
      }
    }
...
export default function EscalaBoard({ config }: { config: EscalaConfig }) {
  ...
  const [state, setState] = useState<EscalaState>(() => defaultState(config));
      ...
      try {
        ...
        if (!error && data) {
          ...
          if (!cancelled && parsed && Array.isArray(parsed.team)) {
            setState(migrate(parsed, config));
          }
        }
      } catch {
        // keep default
  ...
  const summary = useMemo(() => {
    return team.map(m => {
      ...
      for (const d of days) {
        if (d.sunday || holidaySet.has(d.key)) continue;
  ...
  const exportPdf = () => {
    ...
    const sumHead = ['RESUMO', ...(config.hasGenius ? ['GEN'] : []), 'STD', 'OU', 'Σ', 'FDS'].map(h => `<th>${esc(h)}</th>`).join('');
    const sumRows = summary.map(s =>
      `<tr><td style="text-align:left">${chip(s.id)}</td>${config.hasGenius ? `<td>${s.pg}</td>` : ''}<td>${s.std}</td><td>${s.outros}</td><td style="font-weight:700">${s.total}</td><td>${s.fds}</td></tr>`
    ).join('');
  ...
  const save = async () => {
    ...
    try {
      ...
      const { error } = await supabase.storage
        .from(ESCALA_BUCKET)
        .upload(config.storagePath, blob, { upsert: true, contentType: 'application/json' });
          ...
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr className="bg-muted text-muted-foreground">
                ...
                {config.hasGenius && <th className="px-2 py-1.5 text-center font-semibold" title="Dias como Genius">GEN</th>}
            ...
            <tbody>
              {summary.map(s => (
                <tr key={s.id} className="odd:bg-background even:bg-muted/20">
                  ...
                  {config.hasGenius && <td className="px-2 py-1 text-center tabular-nums">{s.pg}</td>}
